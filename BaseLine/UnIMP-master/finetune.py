...
def finetune_model(args, device=torch.device('cpu')):
    ...
    lm_model, tokenizer, hyperedge, train_hyper_node, train_ve_affiliation, train_labels, test_hyper_node, test_ve_affiliation, test_labels, dataset, chunk_map, train_tokens_emb_LLM, test_node_text, scalers = load_data(args)
            ...
            with torch.no_grad():
                for k in range(len(dataset)):
                    ...
                    if args.header_type == "Linear":
                        ...
                        # 反归一化数据
                        if scalers is not None and scalers[k] is not None:
                            imputed_datasets = scalers[k].inverse_transform(imputed_datasets)

                        # 读取源文件的列名
                        
                        source_file = f"./data/{dataset[k]}.csv"
                        source_df = pd.read_csv(source_file, nrows=0)  # 只读取列名
                        column_names = list(source_df.columns)

                        # 创建包含列名和数据的DataFrame
                        df_result = pd.DataFrame(imputed_datasets, columns=column_names)
                        # df_result.to_csv(f"/home/extra_home/lc/DDGANI/from_unimp/{dataset[k]}_filled.csv", index=False, float_format='%.2f')
                        if args.mr:
                            df_result.to_csv(f"/home/extra_home/lc/UnIMP-master/miss_type/{args.missing_ratio}_{dataset[k]}_filled.csv", index=False)
                        else:
                            df_result.to_csv(f"/home/extra_home/lc/UnIMP-master/miss_type/{args.missing_mechanism}_{dataset[k]}_filled.csv", index=False)
