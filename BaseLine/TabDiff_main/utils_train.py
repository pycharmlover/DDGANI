...
def preprocess(dataset_path, y_only=False, dequant_dist='none', int_dequant_factor=0.0, task_type = 'binclass', inverse = False, cat_encoding = None, concat = True):
    ...
    T = TabDiff_main.src.Transformations(**T_dict)
    ...
    if cat_encoding is None:
        ...
        categories = TabDiff_main.src.get_categories(X_train_cat)
...
def make_dataset(
    data_path: str,
    T: TabDiff_main.src.Transformations,
    task_type,
    change_val: bool,
    concat = True,
    y_only = False,
    ...
    if task_type == 'binclass' or task_type == 'multiclass':
        ...
        for split in ['train', 'test']:
            X_num_t, X_cat_t, y_t = TabDiff_main.src.read_pure_data(data_path, split)
            if y_only:
                X_num_t = X_num_t[:, :0]
                X_cat_t = X_cat_t[:, :0]
    ...
    info = TabDiff_main.src.load_json(os.path.join(data_path, 'info.json'))
    int_col_idx_wrt_num = info['int_col_idx_wrt_num']

    if y_only:
        int_col_idx_wrt_num = []
    D = TabDiff_main.src.Dataset(
        X_num,
        X_cat,
        y,
        int_col_idx_wrt_num,
        y_info={},
        task_type=TabDiff_main.src.TaskType(info['task_type']),
        n_classes=info.get('n_classes')
    )

    if change_val:
        D = TabDiff_main.src.change_val(D)

    return TabDiff_main.src.transform_dataset(D, T, None)