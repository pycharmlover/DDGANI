...
class UnifiedCtimeDiffusion(torch.nn.Module):
    ...
    #     """
    #     # If there are no categorical features, return the input directly
    #     if len(self.num_classes) == 0:
    #         return x, x
            
    #     if strategy == 'hard':
    #         move_indices = torch.rand(
    #         * x.shape, device=x.device) < move_chance.unsqueeze(1)
    ...
    #     num_classes_tensor = torch.tensor(self.num_classes, device=x_cat.device).expand_as(x_cat)
    #     x_cat_clamped = torch.minimum(torch.maximum(x_cat, torch.zeros_like(x_cat)), num_classes_tensor - 1)
    #     # x_cat_clamped = torch.clamp(x_cat, 0, torch.tensor(self.num_classes, device=x_cat.device))
    #     x_cat_oh = torch.cat(
    #         [F.one_hot(x_cat_clamped[:, i], num_classes=self.num_classes[i]+1,) for i in range(len(self.num_classes))], 
    #         dim=-1
    #     )
    #     return x_cat_oh
    def to_one_hot(self, x_cat):
    ...
    def _absorbed_closs(self, model_output, x0, sigma, dsigma):
        ...
        # print("x0 shape:", x0_cpu.shape)
        # print("x0 min:", x0_cpu.min().item())
        # print("x0 max:", x0_cpu.max().item())
    ...
    def sample_impute(self, x_num, x_cat, num_mask_idx, cat_mask_idx, resample_rounds, impute_condition, w_num, w_cat):
        ...
        if impute_condition == "x_t":
            z_norm = x_num.to(device) + torch.randn((b, self.num_numerical_features), device=device) * sigma_num_cur[-1]   # z_{t_max} = x_0(masked) + sigma_max*epsilon
        ...
        for i in pbar:
            for u in range (resample_rounds):
                ...
                if impute_condition == "x_t":
                    z_norm_known = x_num.to(device) + torch.randn((b, self.num_numerical_features), device=device) * sigma_num_next[i]
                    ...
                    # z_cat_known, _ = self.q_xt(x_cat.to(device), move_chance)
                elif impute_condition == "x_0":
                    z_norm_known = x_num
                    z_cat_known = x_cat
                ...
                z_norm = (1 - num_mask.to(device))  * z_norm_known + num_mask.to(device) * z_norm_unknown
                # z_cat = (1 - cat_mask.to(device)) * z_cat_known + cat_mask.to(device) * z_cat_unknown
