...
# Metrics
from TabDiff_main.eval.mle.mle import get_evaluator
from TabDiff_main.eval.visualize_density import plot_density
...
class TabMetrics(object):
    ...
    def evaluate_density(self, syn_data):
        ...
        if y_only:
            target_col_idx = self.target_col_idx[0]
...
def reorder(real_data, syn_data, num_col_idx, cat_col_idx, target_col_idx, task_type, metadata):
    num_col_idx = deepcopy(num_col_idx)
    cat_col_idx = deepcopy(cat_col_idx)
    target_col_idx = deepcopy(target_col_idx)
    ...
    for i in range(len(new_real_data.columns)):
        if i < len(num_col_idx):
            new_columns[i] = columns[num_col_idx[i]]
        else:
            new_columns[i] = columns[cat_col_idx[i - len(num_col_idx)]]
