...
class Tokenizer(nn.Module):
    ...
    def __init__(self, d_numerical, categories, d_token, bias):
        ...
        if categories is None:
            ...
            # self.cat_weight = nn.Parameter(torch.randn(sum(categories), d_token))
    ...
    def forward(self, x_num, x_cat):
        ...
        if x_num is not None:
            ...
            x = torch.zeros(len(x_some), 1, self.weight.shape[1], device=x_some.device)
        ...
        if x_cat is not None:
            for start, end in zip(self.category_offsets, torch.cat([self.category_offsets[1:], torch.tensor([x_cat.shape[1]], device=x_cat.device)])):
                if start < end:
                    cat_emb = x_cat[:, start:end].unsqueeze(1) @ self.cat_weight[start:end][None]
