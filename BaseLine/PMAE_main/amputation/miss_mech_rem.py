...
def simulate_nan(
    ...
    if mecha == "MAR":
        mask = MAR_mask(X, p = p_miss, p_obs = p_obs, sample_columns=True).astype(float)
    elif mecha == "MNAR" and opt == "logistic":
        mask = MNAR_mask_logistic(X, p_miss, p_obs, exclude_inputs = False).astype(float)
...
def ampute(
    ...
    if mechanism == 'MAR':
        x_simulated = simulate_nan(
            x.values, p_miss, mechanism, p_obs = 1 - p_miss_c, sample_columns=sample_columns
        )
        mask = pd.DataFrame(x_simulated["mask"])
        x_miss = pd.DataFrame(x_simulated["X_incomp"])
        
    else:
        x_simulated = simulate_nan(
            x[sampled_columns].values, p_miss, mechanism, p_obs = 1 - p_miss_c, sample_columns=sample_columns
        )
