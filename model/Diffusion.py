...
import numpy as np
from model.CFD_model import get_my_CFD_loss, get_my_CFD_loss_trainable, get_my_CFD_loss, update_CFD_models, update_CFD_models, get_eq_dict, get_trueProObserve, split_rule_models, save_cfd_rules
...
def train_one_epoch_Generator(model, discriminator_noise_x,DS, x, optimizer, loss_scaler,other_loss, epoch, total_epochs, timesteps, device, field,M,m_data,loss_weight,model_list, fields,zero_feed_data, use_fd=False, use_cfd=False):
    ...
    for epo in range(epoch_num):
        ...
        with amp.autocast():
            ...
            rule_loss = Diffusion_rule_loss(decoder_z_impute, model_list, fields, use_fd=use_fd, use_cfd=use_cfd)
            loss = Res_loss * loss_weight['Res_weight'] + G_D_loss * loss_weight['G_D_weight'] + rule_loss * loss_weight['CFD_weight'] + other_loss
...
def Diffusion_FD_loss(Pred_x0, model_list, fields):
    return get_my_CFD_loss(model_list, Pred_x0, fields)


def Diffusion_rule_loss(Pred_x0, model_list, fields, use_fd=False, use_cfd=False):
    if len(model_list) == 0:
        return 0
    CFD_models, CFD_models = split_rule_models(model_list)
    loss = 0
    if use_fd and len(CFD_models) > 0:
        loss = loss + get_my_CFD_loss_trainable(CFD_models, Pred_x0, fields)
    if use_cfd and len(CFD_models) > 0:
        loss = loss + get_my_CFD_loss(CFD_models, Pred_x0, fields)
    return loss
...
def train_diffusion_discriminator(discriminator, generator_x,discriminator_noise_x, model_list, num_steps,epochs, lr, batch_size, loss_weight, data_m, impute_data_code, label_data, fields, value_cat, values,miss_data_x,enc,ori_data,continuous_cols,label_num,device,use_Learner,args=None):
    print("------------------Diffusion Discriminator train--------------------")
    use_fd = _is_true_flag(getattr(args, "UseFD", "False"))
    use_cfd = _is_true_flag(getattr(args, "UseCFD", "False"))
    ...
    L_loss = 0
    cost_time = 0
    best_impute_data = None
    r_M = 0x3f3f3f3f
    b_e = 0
    for epoch in tqdm(range(1, total_epochs+1)):
        other_loss = float(L_loss) * loss_weight['L_weight']
        generator_x.train()
        train_one_epoch_Generator(generator_x, discriminator_noise_x, DS, x, optimizer_G, GradScaler(),other_loss, epoch, total_epochs, num_steps, device, fields,M_tensor,m_data,loss_weight,model_list, fields,zero_feed_data, use_fd=use_fd, use_cfd=use_cfd)
        ...
        if epoch % 5 == 0:
            ...
            if len(model_list) != 0 and (use_fd or use_cfd):
                ...
                new_model_list = []
                if use_fd and len(CFD_models) > 0:
                    CFD_models = update_CFD_models(generator_data, zero_feed_data, fields, data_m, M_tensor, value_cat,
                                                 values, miss_data_x.copy(), enc, device, eq_dict, CFD_models,
                                                 cell_acc, continuous_cols, cost_time)
                if use_cfd and len(CFD_models) > 0:
                    CFD_models = update_CFD_models(generator_data, zero_feed_data, fields, data_m, M_tensor, value_cat,
                                                   values, miss_data_x.copy(), enc, device, CFD_models,
                                                   cell_acc, continuous_cols, cost_time)
                    # after duplicate, save the valid CFDs
                    cfd_rule_list = []
                    for model in CFD_models:
                        if hasattr(model, "cfd_rule"):
                            cfd_rule_list.append(model.cfd_rule)

                    save_cfd_rules(cfd_rule_list, save_dir="out/CFD_list/", file_name="cfd_rules_updated.txt")

                new_model_list.extend(CFD_models)
                new_model_list.extend(CFD_models)
            ...
            if cur_r < r_M:
                ...
                best_impute_data = reconvert_data(code, fields, value_cat, values, miss_data_x.copy(), data_m, enc)
                best_impute_data = pd.DataFrame(best_impute_data)
                best_impute_data.columns = values
                b_e = epoch
                r_M = cur_r
    
    if best_impute_data is None:
        generator_data, discriminator_out = reverse_diffusion_G_D_sample(
            generator_x, discriminator_noise_x, DS,
            num_steps, no, dim, device, x, M_tensor
        )
        code = zero_feed_data + (1 - M_tensor) * generator_data
        best_impute_data = reconvert_data(code, fields, value_cat, values, miss_data_x.copy(), data_m, enc)
        best_impute_data = pd.DataFrame(best_impute_data)
        best_impute_data.columns = values
    return best_impute_data.values
